...
#os.chdir("C:/Users/90813/Desktop/Taifex_Crawler")
...
def get_interval(datestart,dateend,id):
    ...
    para = '?datestart='+datestart+'&dateend='+dateend+'&COMMODITY_ID='+id
    url = 'http://www.taifex.com.tw/chinese/3/3_1_2dl.asp' + para
...
def update_recent(id):    
    ...
    try:
        if( os.path.isfile("current_data/"+id+"1.csv")):
            ...
            TX2 = TX2_past.append(TX1).drop_duplicates()
            TX3 = TX3_past.append(TX1).drop_duplicates()
            TX4 = TX4_past.append(TX1).drop_duplicates()
            TX5 = TX5_past.append(TX1).drop_duplicates()
    except:    
        print("Error")
...
if(mode == "recent"):
    update_recent(id)
    print("  From R: ",id," Total_OI ~~ Completed\n")
elif(mode=="history"):
    history_data(id)
    update_recent(id)
    print("  From R: ",id," Total_OI ~~ Completed\n")
